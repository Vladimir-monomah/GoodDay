...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['page_name'])) {
        $valid = 0;
        $error_message .= "Имя страницы не может быть пустым<br>";
    } else {
    	...
    	if($total) {
    		$valid = 0;
        	$error_message .= 'Название страницы уже существует<br>';
    	}
    }
...
<section class="content-header">
	<div class="content-header-left">
		<h1>Редактирвоать страницу</h1>
	</div>
	<div class="content-header-right">
		<a href="page.php" class="btn btn-primary btn-sm">Посмотреть все</a>
	</div>
</section>
        ...
        <div class="box box-info">
            ...
            <div class="box-body">
                <div class="form-group">
                    <label for="" class="col-sm-2 control-label">Название страницы <span>*</span></label>
                    <div class="col-sm-4">
                        <input type="text" class="form-control" name="page_name" value="<?php echo $page_name; ?>">
                    </div>
                </div>
                <div class="form-group">
                    <label for="" class="col-sm-2 control-label">Раздел страницы</label>
                    <div class="col-sm-4">
                        <input type="text" class="form-control" name="page_slug" value="<?php echo $page_slug; ?>">
                    </div>
                </div>
                <div class="form-group">
					<label for="" class="col-sm-2 control-label">Макет страницы </label>
					<div class="col-sm-2">
						<select class="form-control select2" name="page_layout" style="width:300px;" onchange="showContentInputArea(this)">
							<option value="Full Width Page Layout" <?php if($page_layout=='Full Width Page Layout') {echo 'selected';} ?>>Макет страницы во всю ширину</option>
							<option value="About Us Page Layout" <?php if($page_layout=='About Us Page Layout') {echo 'selected';} ?>>О нас Макет страницы</option>
							<option value="Service Page Layout" <?php if($page_layout=='Service Page Layout') {echo 'selected';} ?>>Макет служебной страницы</option>
							<option value="FAQ Page Layout" <?php if($page_layout=='FAQ Page Layout') {echo 'selected';} ?>>Макет страницы FAQ</option>
							<option value="Chef Page Layout" <?php if($page_layout=='Chef Page Layout') {echo 'selected';} ?>>Макет страницы шеф-повара</option>
							<option value="Menu Style 1 Page Layout" <?php if($page_layout=='Menu Style 1 Page Layout') {echo 'selected';} ?>>Стиль меню 1 Макет страницы</option>
							<option value="Menu Style 2 Page Layout" <?php if($page_layout=='Menu Style 2 Page Layout') {echo 'selected';} ?>>Стиль меню 2 Макет страницы</option>
							<option value="Menu Style 3 Page Layout" <?php if($page_layout=='Menu Style 3 Page Layout') {echo 'selected';} ?>>Стиль меню 3 Макет страницы</option>
							<option value="Menu Style 4 Page Layout" <?php if($page_layout=='Menu Style 4 Page Layout') {echo 'selected';} ?>>Стиль меню 4 Макет страницы</option>
							<option value="Photo Gallery Page Layout" <?php if($page_layout=='Photo Gallery Page Layout') {echo 'selected';} ?>>Макет страницы фотогалереи</option>
							<option value="News Page Layout" <?php if($page_layout=='News Page Layout') {echo 'selected';} ?>>Макет страницы новостей</option>
							<option value="Contact Us Page Layout" <?php if($page_layout=='Contact Us Page Layout') {echo 'selected';} ?>>Свяжитесь с нами Макет страницы</option>
							<option value="Reserve Table Page Layout" <?php if($page_layout=='Reserve Table Page Layout') {echo 'selected';} ?>>Макет страницы резервной таблицы</option>
						</select>
					</div>
				</div>
                <div class="form-group" id="showPageContent" style="<?php if($page_layout=='News Page Layout' || $page_layout=='About Us Page Layout') {echo 'display:none';}else{echo 'display:block;';} ?>">
					<label for="" class="col-sm-2 control-label">Содержание страницы </label>
					<div class="col-sm-9">
						<textarea class="form-control" name="page_content" id="editor1"><?php echo $page_content; ?></textarea>
					</div>
				</div>
				<div class="form-group">
					<label for="" class="col-sm-2 control-label">Существующий баннер</label>
